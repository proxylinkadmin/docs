...
- The site gateway must forward directed broadcasts: Linux gateways deployed after July 2026 are configured automatically; older Linux gateways need `sysctl -w net.ipv4.conf.all.bc_forwarding=1` (persist it in `/etc/sysctl.d/`). Not currently supported when the tunnel terminates directly on a MikroTik router.
