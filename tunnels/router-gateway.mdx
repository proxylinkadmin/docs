...
<Warning>
Wake-on-LAN is **not currently supported when the tunnel terminates directly on a MikroTik router**, and Linux gateways deployed before late June 2026 need `bc_forwarding=1` enabled. See [Wake-on-LAN](/tunnels/wake-on-lan) for the per-platform requirements.
</Warning>
