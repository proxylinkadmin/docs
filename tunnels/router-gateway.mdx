...
| OpenWRT | WireGuard package (`luci-proto-wireguard`) |
| Linux (Debian, Ubuntu, Raspberry Pi) | Install script |
...
/ip firewall mangle add chain=prerouting action=accept src-address=192.168.1.0/24 dst-address=10.100.0.0/24 comment="ProxyLink"
/ip firewall mangle add chain=prerouting action=accept src-address=10.100.0.0/24 dst-address=192.168.1.0/24 comment="ProxyLink"
```
