...
<Note>
**RDP (3389) and VNC (5900) are intentionally not exposed as raw TCP links.** Those protocols are served through ProxyLink's authenticated browser terminals instead, so raw TCP can't bypass the ProxyLink login gate. Use the in-browser [RDP](/terminals/rdp) and [VNC](/terminals/vnc) instead of a TCP link for those ports.
</Note>
